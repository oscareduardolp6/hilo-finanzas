...
2. **Helpers** — ya no queda ninguno: los últimos (`initialFormState`, `computePeriodTransactions`, `computeRecentTxns`, `computeKnownStores`, `computePlanProgress`) migraron a sus features en el paso 4. *(Antes: `uid`, las fechas, `formatMoney`, la búsqueda y `groupByDate` → `src/shared/domain/`; `highlightMatch` → `src/shared/ui/`; los saldos → `src/features/accounts/domain/`.)*
3. **Shared pieces** — `GlobalStyles` (fonts, scrollbar hiding, sheet animations), `Toast`, `ExpenseDonut` (+ `DonutTooltip`), `BottomNav`. *(Los presentacionales que ya usan varias features viven en `src/shared/ui/`: `SheetOverlay`, `EmptyState`, `CategoryPicker`, `StoreInput`, `AccountChips`, `InstallmentPlanPicker`, `TransactionRow`, `MsiPlanCard` — ver la regla en el plan.)*
4. **Views** (one per bottom-nav tab) — `HomeView`, `HistoryView`. *(Las de cuentas y MSI ya son features: ver abajo.)*
5. **Modals/sheets** — `SettingsModal`, `MonefyImportModal`, `ReceiptScanModal` (photo → Anthropic vision API → review sheet → many transactions), `SyncModal` (manual cross-device merge; tabs *Enviar* / *Recibir* / *Dispositivos*, the last managing this device's name and the per-peer sync points that drive delta sends), `BackupModal` (export / restore-by-replace).
6. **`App`** (default export) — hoy es solo un envoltorio sobre `HiloStoreProvider`; el árbol vive en `AppBody`, que **ya no es dueño de ningún estado**: lo lee entero del store de zustand (paso 2) y sigue bajándolo como props a lo que aún no se ha migrado. Los derivados que quedan (`periodTransactions`, `categoryTotals`, `historySuggestions`…) siguen ahí como `useMemo` hasta que su feature se lleve el suyo, como ya hicieron `accounts` con los saldos y `transactions` con las tiendas conocidas.

### Desktop layout

Above `max-w-md` on a narrow viewport (< 1024px), `App` renders the mobile tree described above unchanged. At `>= 1024px` (`useIsDesktop()`, a `matchMedia` hook), `App` early-returns a **separate, parallel component tree** instead: `DesktopShell` (sidebar nav + wide main area) composing `DesktopSidebar` and `HomeViewDesktop`/`HistoryViewDesktop` — desktop-specific layouts (multi-column grids, more visible at once) that accept the *same props* as their mobile counterparts, so `App`'s state/derived-data layer doesn't change at all between the two. `SheetOverlay` and `Toast` take a `desktop` prop to switch from mobile bottom-sheet/toast positioning to a centered modal / corner toast; every modal component (`SettingsModal`, `MonefyImportModal`, `ReceiptScanModal`, `SyncModal`, `BackupModal`) just forwards it through. See [tasks/desktop-view.md](tasks/desktop-view.md) and [agents/plans/desktop-view.md](agents/plans/desktop-view.md) for why a separate tree was chosen over a single responsive one.

**Lo que cambia según se migra cada feature:** una feature ya migrada no aparece dos veces en ese cableado. Los dos árboles montan **el mismo container** y le pasan `desktop`; el container elige la vista y se sirve del store, sin recibir props. Así lo hacen `accounts` (`AccountsContainer` / `AccountFormContainer`), `transactions` (`AddTransactionContainer`) e `installments` (`MsiContainer` / `MsiPlanFormContainer`), que entre las tres le han quitado ~25 props a `DesktopShell`. El paso 12 termina de vaciarlo.
